...
npm run install:all
```
*(This uses our dynamic script from the root `scripts/install-all.js` to install dependencies for `main-app` and all sub-apps found in `apps/`)*
...
npm run dev:all
```
*(Starts the main shell on port 3000, and fully boots all frontend and backend servers dynamically based on what it finds in `apps/`)*
